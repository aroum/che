...
function Rails:new(chunks, tab, active)
...
function Rails:build()
	...
	if c[1].w > 0 then
		children[#children + 1] = Rail:new("rail-left", c[2] { w = math.min(1, c[2].w) }, c, self._active)
	end
	if c[3].w > 0 then
		children[#children + 1] =
			Rail:new("rail-right", c[2] { x = math.max(0, c[2].right - 1), w = math.min(1, c[2].w) }, c, self._active)
	end
